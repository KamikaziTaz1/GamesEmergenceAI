...
public class Grammar : MonoBehaviour {
   ...
   for(int i = 0; i < symbols.Length; i++){
       RandFuncs.shuffle(expansionRules);
       for(int j = 0; j < expansionRules.Length; j++){
           ...
           if(expandInParallel == true){
               if(symbols[i] == expansionRules[j].beforeString){
                   symbols[i] = expansionRules[j].afterString;
                }
           }else if (expandInParallel == false){
            if(symbols[i] == expansionRules[j].beforeString){
                symbols[i] = expansionRules[j].afterString;
                return;
            }
